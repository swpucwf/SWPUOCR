...
from utils.cptn_utils import gen_anchor, bbox_transfor_inv, clip_box, filter_bbox, nms, TextProposalConnectorOriented, \
    resize
...
weights = os.path.join(cptn_config.checkpoints_dir, 'CTPN.pth')
model = CTPN_Model()
...
def get_det_boxes(image,display =True, expand = True):
    ...
    image_c = image.copy()
    h, w = image.shape[:2]
    image = image.astype(np.float32) - cptn_config.IMAGE_MEAN
